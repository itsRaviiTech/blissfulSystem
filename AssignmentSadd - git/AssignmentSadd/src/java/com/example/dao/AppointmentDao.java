...
public class AppointmentDao {
    ...
    public static List<Appointment> findByUser(int userId) throws SQLException {
        String sql =
          "SELECT a.*, s.name AS staffName FROM appointments a " +
          " LEFT JOIN staff s ON a.staff_id=s.staff_id " +
          " WHERE a.name=(SELECT u.name FROM users u WHERE u.id=?)";
          ...
          List<Appointment> list = new ArrayList<>();
            while(rs.next()) {
                ...
                a.setAddons(rs.getString("addons"));
    ...
    public static void create(Appointment a) throws SQLException {
        String sql = "INSERT INTO appointments "
                   + "(appt_date,appt_time,service,addons,price,status,name,phone,staff_id) "
                   + "VALUES(?,?,?,?,?,'Pending',?,?,?)";
        try(Connection c = DatabaseConnection.getConnection();
        PreparedStatement p=c.prepareStatement(sql)) {
            p.setDate(1,a.getApptDate());
            p.setTime(2,a.getApptTime());
            p.setString(3,a.getService());
            p.setString(4,a.getAddons());
            p.setBigDecimal(5,a.getPrice());
            p.setString(6,a.getName());
            p.setString(7,a.getPhone());
            p.setInt(8,a.getStaffid());
            p.executeUpdate();
        }
    }
}
